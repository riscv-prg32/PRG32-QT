...
PRG32-QT targets portable ABI-table PRG32 cartridges and follows the attached PRG32-iOS implementation as its behavioral parity baseline.
...
- 139-entry ABI table.
- Provided features exactly matching the iOS baseline: audio, metrics, audio-plus, tilemap, platformer, sprites.
...
`tests/fixtures/Asteroids.prg32` and `tests/fixtures/Bach.prg32` are copied from the attached PRG32-iOS baseline and must both initialize and execute repeated frames in CTest. `Bach` is particularly useful for exercising packaged audio metadata and audio ABI calls.
