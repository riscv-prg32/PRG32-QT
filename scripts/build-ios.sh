...
case "$DESTINATION" in
  *Simulator*) BUILD_DIR=build-ios-sim; SDK=iphonesimulator ;;
  *) BUILD_DIR=build-ios-device; SDK=iphoneos ;;
esac
"$QT/bin/qt-cmake" -S . -B "$BUILD_DIR" -G Xcode -DCMAKE_SYSTEM_NAME=iOS -DCMAKE_OSX_SYSROOT="$SDK" -DCMAKE_OSX_ARCHITECTURES=arm64 -DQT_HOST_PATH="$QT_HOST" -DPRG32QT_BUILD_APP=ON -DPRG32QT_BUILD_TESTS=OFF
