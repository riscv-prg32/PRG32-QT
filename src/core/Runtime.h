...
public:
 ...
 bool load(const Cartridge&,std::string&);bool init(std::string&);bool frame(uint32_t,std::string&);void stop();
...
private:
 void hostCall(uint32_t);uint32_t guestPtr(uint32_t off)const{return base_+off;}std::string guestString(uint32_t,size_t=1024)const;bool writeCString(uint32_t,const std::string&,size_t);void writeAbiTable();uint64_t nowUs()const;void pulseLED(int=69,uint8_t=180);
 ...
 struct Score{std::string game,player;uint32_t score;};std::vector<Score>scores_;std::string currentPlayer_="PLAYER";int scoreGet(const std::string&,int,uint32_t);
