...
void FrameItem::paint(QPainter*p){p->setRenderHint(QPainter::SmoothPixmapTransform,false);p->fillRect(boundingRect(),Qt::black);if(!image_.isNull()){QSizeF size=image_.size();size.scale(boundingRect().size(),Qt::KeepAspectRatio);QRectF dest(QPointF((width()-size.width())/2,(height()-size.height())/2),size);p->drawImage(dest,image_);}}
