...
public:
    ...
    void setState(bool connected, QString name) {
        if (connected_ == connected && name_ == name) return;
        connected_ = connected; name_ = std::move(name); emit connectedChanged();
    }
    void publish(std::uint32_t mask) { emit inputChanged(mask); }
signals:
