...
void WebApiServer::serve(QTcpSocket* socket, const QByteArray& request) {
    ...
    if (first.size() < 2)
        ...
        if (method == "GET" && (path == "/api" || path == "/api/")) {
            ...
            for (const auto& entry : {qMakePair("GET", "/api"),
                                      qMakePair("GET", "/api/"),
                                      qMakePair("GET", "/api/runtime"),
                                      qMakePair("GET", "/api/games"),
                                      qMakePair("POST", "/api/games"),
                                      qMakePair("POST", "/api/games/select"),
                                      qMakePair("GET", "/api/screenshot.bmp"),
            ...
            json(controller_->runtimeJson());
        else if (method == "GET" && path == "/api/games")
            json(controller_->gamesJson());
        else if (method == "GET" && path == "/api/performance.json")
            ...
            mime = "image/bmp";
            output = controller_->screenshotBmp();
        } else if (method == "POST" && path == "/api/games") {
            QString error;
            if (!controller_->uploadSlot(slot, body, error)) {
                status = 400;
                json(QJsonObject{{"ok", false}, {"error", error}});
            } else
                json(QJsonObject{{"ok", true},
                                 {"slot", QString("cart%1").arg(slot)},
                                 {"stored", true},
                                 {"loaded", false}});
        } else if (method == "POST" && path == "/api/games/select") {
            QString error;
            if (!controller_->selectSlot(slot, error)) {
                status = 400;
                json(QJsonObject{{"ok", false}, {"error", error}});
            } else
                json(QJsonObject{{"ok", true}, {"slot", QString("cart%1").arg(slot)}});
        } else if (method == "POST" && path == "/api/scores") {
            QString error;
            auto o = QJsonDocument::fromJson(body).object();
            if (!controller_->submitScore(o, error)) {
                status = 400;
                json(QJsonObject{{"ok", false}, {"error", error}});
            } else
                json(QJsonObject{{"ok", true}});
